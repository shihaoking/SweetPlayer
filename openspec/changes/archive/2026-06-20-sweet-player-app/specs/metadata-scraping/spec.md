...
### 需求：刮削自动触发机制
系统应在文件源扫描完成后自动触发刮削任务，实现文件发现到元数据获取的自动化流程。

#### 场景：扫描发现新文件自动刮削

- **WHEN** 文件源扫描完成，检测到新增或删除的视频文件
- **THEN** 系统自动将所有新增的视频文件加入刮削队列
- **AND** 后台刮削服务立即开始处理队列中的文件
- **AND** 每个文件刮削完成后延迟 2-5 秒再处理下一个文件（防止 API 限流）

#### 场景：扫描无文件变化

- **WHEN** 文件源扫描完成，未检测到新增或删除的文件
- **THEN** 系统不触发刮削任务

#### 场景：手动触发刮削

- **WHEN** 用户在主屏幕点击"刮削影片"按钮
- **THEN** 系统查询数据库中所有未刮削的视频文件（MovieMetadataId 为 null）
- **AND** 将这些文件批量加入刮削队列
- **AND** 后台刮削服务立即开始处理
- **AND** 按钮显示加载状态，刮削进行时禁用按钮

#### 场景：应用启动不自动刮削

- **WHEN** 应用启动时
- **THEN** 系统启动刮削队列后台服务（等待任务）
- **BUT** 不自动加载未刮削的文件
- **AND** 等待扫描触发或用户手动触发
